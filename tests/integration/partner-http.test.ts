...
it('keeps selected partner note filters, totals and missing-stock authorization inside one read seam', async () => {
 const a = await login(), b = await login()
 const { link } = await (await a.browser.post('/api/partners', { partnerEmail: b.email })).json()
 const targetId = link.partner.id
 const first = await (await b.browser.post('/api/stocks/AAPL/notes', { title: 'First partner note', content: 'First', date: '2026-02-01T00:00:00Z' })).json()
 await b.browser.post('/api/stocks/AAPL/notes', { title: 'Second partner note', content: 'Second', date: '2026-02-01T00:00:00Z' })
 const agent = await (await b.browser.post('/api/stocks/AAPL/notes', { title: 'Agent partner note', content: 'Agent', date: '2026-02-01T00:00:00Z' })).json()
 await database.pool.query("update stock_notes set created_via = 'AGENT', created_by_label = 'Synthetic partner agent' where id = $1", [agent.id])
 const path = `/api/stocks/AAPL/notes?partnerId=${targetId}`
 expect((await a.browser.request(`/api/stocks/NEVEREXISTS/notes?partnerId=${targetId}`)).status).toBe(403)
 await b.browser.post(`/api/partners/${link.id}/accept`, {})
 await mutate(b.browser, `/api/partners/${link.id}/sharing`, { shareStockNotes: true })
 const pageOne = await a.browser.request(`${path}&createdVia=USER&page=1&limit=1`)
 expect(pageOne.status).toBe(200)
 const firstPage = await pageOne.json()
 expect(firstPage.pagination).toEqual({ page: 1, limit: 1, total: 2, totalPages: 2 })
 expect(firstPage.data[0]).toMatchObject({ title: 'Second partner note', isOwnedByViewer: false })
 const pageTwo = await (await a.browser.request(`${path}&createdVia=USER&page=2&limit=1`)).json()
 expect(pageTwo.data[0]).toMatchObject({ id: first.id, title: 'First partner note', isOwnedByViewer: false })
 const filtered = await (await a.browser.request(`${path}&createdVia=AGENT`)).json()
 expect(filtered.pagination).toMatchObject({ total: 1, totalPages: 1 }); expect(filtered.data[0]).toMatchObject({ title: 'Agent partner note', createdByLabel: 'Synthetic partner agent' })
 const allowedMissingStock = await a.browser.request(`/api/stocks/NEVEREXISTS/notes?partnerId=${targetId}`)
 expect(allowedMissingStock.status).toBe(200); expect((await allowedMissingStock.json()).data).toEqual([])
})

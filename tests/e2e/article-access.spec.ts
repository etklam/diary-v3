...
test('article access protects SSR, login returns, logout, editor transitions, and mobile reading', async ({ page, browser }) => {
  ...
  try {
    ...
    await reader.reload()
    await expect(reader.locator('.safe-markdown')).toContainText(`Public research ${key}`)
    const locked = await reader.goto(memberPath)
    ...
    expect((await reader.request.post('/api/auth/register', { data: { email, password } })).status()).toBe(200)
    await reader.locator('.article-lock').getByRole('link', { name: 'Sign in', exact: true }).click()
    ...
    await expect(reader).toHaveURL(new RegExp(`${memberPath}$`))

...
test('two independent accounts accept, share separately and remove a partnership', async ({ page: a, browser, playwright }) => {
 ...
 async function signIn(page: Page, email: string) {
  await page.goto('http://127.0.0.1:3200/login?returnTo=%2Fpartners'); await page.getByTestId('locale-select').selectOption('en');
  await page.getByLabel('Email', { exact: true }).fill(email); await page.getByLabel('Password', { exact: true }).fill(password);
  await page.getByRole('button', { name: 'Sign in', exact: true }).click(); await expect(page).toHaveURL(/\/partners$/); await page.getByTestId('locale-select').selectOption('en');
 }
 ...
 try {
  ...
  await a.getByTestId('sign-out').click(); await expect(a.getByTestId('sign-out')).toHaveCount(0);
  ...
  await b.getByTestId('theme-select').selectOption('dark');
  ...
  for (const [locale, title] of [['zh-TW', '日記對照'], ['zh-CN', '日记对照'], ['en', 'Pair View']] as const) { await a.getByTestId('locale-select').selectOption(locale); await expect(a.getByRole('heading', { name: title, exact: true })).toBeVisible(); await expect(a.getByTestId('partner-diary')).toContainText('Partner comparison entry'); }
  ...
  await a.getByTestId('sign-out').click(); await expect(a.getByLabel('Partner email', { exact: true })).toHaveCount(0);
...
test('comparison login preserves an explicit partner selection', async ({ page }) => {
 ...
 await page.goto('/partners/compare?partnerId=999999'); await page.getByTestId('locale-select').selectOption('en');

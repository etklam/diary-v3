...
test('Review queue keeps all open diaries reachable across pages', async ({ page, context }) => {
  ...
  for (let day = 1; day <= 22; day++) expect((await page.request.post('/api/diaries', { headers, data: { title: `Upcoming entry ${day}`, date: `2026-02-${String(day).padStart(2, '0')}`, content: 'Synthetic', reviewDueAt: '2099-01-01T00:00:00Z' } })).status()).toBe(201);
  await page.goto('/reviews'); const overdue = page.getByRole('region', { name: 'Overdue', exact: true }), upcoming = page.getByRole('region', { name: 'Upcoming', exact: true });
  await expect(overdue.getByTestId('review-queue-item')).toHaveCount(20); await expect(upcoming.getByTestId('review-queue-item')).toHaveCount(20); await expect(page.getByTestId('queue-count-overdue')).toContainText('25');
  ...
  await overdue.getByRole('navigation', { name: 'Overdue Pagination', exact: true }).getByRole('button', { name: 'Next page', exact: true }).click(); await expect(page).toHaveURL(/\/reviews\?overduePage=2$/); await expect(overdue.getByTestId('review-queue-item')).toHaveCount(5); await expect(upcoming.getByTestId('review-queue-item')).toHaveCount(20); await expect(overdue.getByTestId('review-queue-item').filter({ hasText: 'Queue entry 21' })).toHaveCount(1); await expect(page.getByTestId('queue-count-overdue')).toContainText('25');
  await upcoming.getByRole('navigation', { name: 'Upcoming Pagination', exact: true }).getByRole('button', { name: 'Next page', exact: true }).click(); await expect(page).toHaveURL(/\/reviews\?overduePage=2&upcomingPage=2$/); await expect(overdue.getByTestId('review-queue-item')).toHaveCount(5); await expect(upcoming.getByTestId('review-queue-item')).toHaveCount(2);
  await page.reload(); await expect(overdue.getByTestId('review-queue-item').filter({ hasText: 'Queue entry 21' })).toHaveCount(1); await expect(upcoming.getByTestId('review-queue-item')).toHaveCount(2); await overdue.getByRole('navigation', { name: 'Overdue Pagination', exact: true }).getByRole('button', { name: 'Previous page', exact: true }).click(); await expect(overdue.getByTestId('review-queue-item')).toHaveCount(20); await expect(upcoming.getByTestId('review-queue-item')).toHaveCount(2);
});

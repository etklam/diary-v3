...
for (const width of [1440, 390]) test(`foreground reminders recover and remain unique at ${width}px`, async ({ page, context }) => {
  ...
  await page.getByRole('button', { name: 'Sign in', exact: true }).click(); await expect(page).toHaveURL(/\/diaries\/new$/); await page.getByTestId('locale-select').selectOption('en'); await connected;
  ...
  await page.getByTestId('sign-out').click(); await expect(notice).toHaveCount(0);
});
...
test('server-side account revocation clears the foreground notice and returns to login', async ({ page, context }) => {
  ...
  await page.getByRole('button', { name: 'Sign in', exact: true }).click(); await expect(page).toHaveURL(/\/diaries\/new$/); await page.getByTestId('locale-select').selectOption('en'); await socket;
  const headers = { 'x-csrf-token': (await context.cookies()).find(cookie => cookie.name === 'csrf-token')!.value };
...
for (const width of [1440, 390]) test(`price trigger hints restore authoritative state at ${width}px`, async ({ page, context }) => {
  ...
  await page.getByLabel('Email', { exact: true }).fill(email); await page.getByLabel('Password', { exact: true }).fill(password);
  const connected = page.waitForEvent('websocket', socket => socket.url().includes('/socket.io/'));
  await page.getByRole('button', { name: 'Sign in', exact: true }).click(); await expect(page).toHaveURL(/\/diaries\/new$/); await connected; await page.getByTestId('locale-select').selectOption('en');
  const headers = { 'x-csrf-token': (await context.cookies()).find(cookie => cookie.name === 'csrf-token')!.value };
...
test('transport reconnect restores an alert triggered while the browser is offline', async ({ page, context }) => {
  ...
  await page.getByTestId('locale-select').selectOption('en'); await expect.poll(() => connections).toBeGreaterThan(0);
  const headers = { 'x-csrf-token': (await context.cookies()).find(cookie => cookie.name === 'csrf-token')!.value };

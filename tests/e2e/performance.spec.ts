...
for (const width of [1440, 390]) test(`Performance filters, tables and charts at ${width}px`, async ({ page, context }) => {
  ...
  await page.reload(); await expect(page.getByTestId('performance-totalRealizedPnL')).toHaveText('10.00'); await expect(page.getByTestId('performance-winRate')).toHaveText('33.33%'); await expect(page.getByRole('table', { name: 'By strategy', exact: true })).toContainText('Growth'); await expect(page.getByRole('table', { name: 'By emotion', exact: true })).toContainText('Calm'); await expect(page.getByRole('img', { name: 'Cumulative realized P&L', exact: true })).toBeVisible();
  ...
  for (const [locale, name] of [['zh-TW', '策略績效'], ['zh-CN', '策略绩效'], ['en', 'Strategy performance']] as const) { await page.getByTestId('locale-select').selectOption(locale); await expect(page.getByRole('heading', { name, exact: true })).toBeVisible(); }
  await page.getByTestId('sign-out').click(); await expect(page.getByTestId('performance-totalClosedTrades')).toHaveCount(0);
});

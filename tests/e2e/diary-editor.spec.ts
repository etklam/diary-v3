...
for(const width of [1440,390]){
 test(`full diary authoring, safe Markdown and deletion at ${width}px`,async({page,context})=>{
  ...
  for(const [locale,heading] of [['zh-TW','編輯日記'],['zh-CN','编辑日记'],['en','Edit diary']] as const){await page.getByTestId('locale-select').selectOption(locale);await expect(page.getByRole('heading',{name:heading,exact:true})).toBeVisible();}

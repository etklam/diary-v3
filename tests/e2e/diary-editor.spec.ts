...
for(const width of [1440,390]){
 test(`full diary authoring, safe Markdown and deletion at ${width}px`,async({page,context})=>{
  ...
  await page.route('**/api/v2/diaries/*',async route=>{if(route.request().method()==='PUT')await route.fulfill({status:500,contentType:'application/json',body:JSON.stringify({data:{code:'SYS_INTERNAL_ERROR',requestId:'editor-failure'}})});else await route.continue();});

...
for (const width of [1440, 390]) test(`Thesis activation review and snapshot at ${width}px`, async ({ page }) => {
  ...
  if (width === 390) await page.getByTestId('theme-select').selectOption('dark'); await page.getByTestId('thesis-review').screenshot({ path: `docs/design/evidence/thesis/review-${width}.png` }); await current.screenshot({ path: `docs/design/evidence/thesis/editor-${width}.png` }); expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
  ...
  for (const [locale, title] of [['zh-TW', 'AAPL · 投資論點'], ['zh-CN', 'AAPL · 投资论点'], ['en', 'AAPL · Investment thesis']] as const) { await page.getByTestId('locale-select').selectOption(locale); await expect(page.getByRole('heading', { name: title, exact: true })).toBeVisible(); }
  await page.getByTestId('sign-out').click(); await expect(page.getByTestId('thesis-review')).toHaveCount(0);
});

...
for (const width of [1440, 390]) {
  test(`Diary library filtering, pagination and recovery at ${width}px`, async ({ page, context }) => {
    ...
    await page.route('**/api/diaries/summary*', async route => route.fulfill({ status: 500, contentType: 'application/json', body: JSON.stringify({ data: { code: 'SYS_INTERNAL_ERROR', requestId: 'library-retry' } }) }))
    await page.getByRole('button', { name: 'Apply filters', exact: true }).click()

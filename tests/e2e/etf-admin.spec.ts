...
test('Admin manages ETF history and catalog at desktop and mobile sizes', async ({ page }) => {
 ...
 await expect(page).toHaveURL(/\/admin\/etf$/); await page.getByTestId('locale-select').selectOption('en');
 ...
 for(const [locale,title] of [['zh-TW','ETF 目錄'],['zh-CN','ETF 目录'],['en','ETF catalog']] as const){await page.getByTestId('locale-select').selectOption(locale);await expect(page.getByRole('heading',{name:title,exact:true})).toBeVisible();}
 ...
 for(const width of [1440,390]) { await page.setViewportSize({width,height:900}); if(width===390)await page.getByTestId('theme-select').selectOption('dark'); expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true); await page.locator('.plan-page').screenshot({path:`docs/design/evidence/etf-admin/${width}.png`}); }
...
test('catalog form keeps 20-character symbols and 255-character names usable by keyboard',async({page})=>{
 await page.goto('/login?returnTo=%2Fadmin%2Fetf');await page.getByTestId('locale-select').selectOption('en');
 await page.getByLabel('Email',{exact:true}).fill('etf-admin@example.test');await page.getByLabel('Password',{exact:true}).fill('synthetic-etf-admin-password');await page.getByRole('button',{name:'Sign in',exact:true}).click();await expect(page).toHaveURL(/\/admin\/etf$/);await page.getByTestId('locale-select').selectOption('en');

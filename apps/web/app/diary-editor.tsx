...
export function DiaryEditor({initial,id,quick=false}:{initial:DiaryFields;id?:string;quick?:boolean}){
 ...
 const [form,setForm]=useState({date:initial.date,title:initial.title,content:initial.content,tags:initial.tags.length?[...initial.tags]:[''],thesis:initial.thesis,risk:initial.risk,execution:initial.execution});const [preview,setPreview]=useState(false);const [pending,setPending]=useState(false);const [error,setError]=useState<Failure|null>(null);const dirty=useRef(false);const [transactions,setTransactions]=useState<BuyDraft[]>(()=>initial.transactions?.map(row=>({key:row.id,id:row.id,type:row.type,symbol:row.symbol,quantity:row.quantity,price:row.price,tradeDate:localTradeValue(new Date(row.tradeDate)),instant:row.tradeDate,notes:row.notes??'',strategy:row.strategy??'',emotion:row.emotion??''}))??[]);const [transactionError,setTransactionError]=useState('');const [stockSymbols,setStockSymbols]=useState((initial.stockSymbols??[]).join(', '));const [reviewTime,setReviewTime]=useState(initial.reviewDueAt?localTradeValue(new Date(initial.reviewDueAt)):'');const [reviewInstant,setReviewInstant]=useState(initial.reviewDueAt??'');const [recoveryState,setRecoveryState]=useState<'conflict'|'unavailable'|null>(null);const [recoveryDiary,setRecoveryDiary]=useState<DiaryResponse|null>(null);

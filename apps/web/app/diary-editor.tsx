...
export function DiaryEditor({initial,id,accountId,quick=false,captureContext,captureIssue:_captureIssue}:{initial:DiaryFields;id?:string;accountId?:string;quick?:boolean;captureContext?:CaptureContext|null;captureIssue?:import('./capture-context').CaptureContextIssue|null}){
 ...
 function restoreDraft(){const draft=restorable!;const merged=mergeDraft(sourcesRef.current,draft,id?sourcesRef.current.stockSymbols:'');if(draft.captureContext)captureRef.current=draft.captureContext;setForm(merged.form);setStockSymbols(merged.stockSymbols);setTransactions(merged.transactions);setReviewTime(merged.reviewTime);setReviewInstant(merged.reviewInstant);setReminders(merged.reminders);setRestorable(null);}

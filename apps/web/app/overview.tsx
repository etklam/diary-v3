...
function ReviewSection({ state, retry, timezone, locale, c }: { state: ResourceState<ReviewGroups>; retry: () => void; timezone: string | null; locale: string; c: OverviewCopy }) {
  return <section className="overview-section" aria-labelledby="overview-review-title"><header className="overview-section-header"><h2 id="overview-review-title"><Icon name="check" />{c.reviewQueue}</h2><Link to="/reviews">{c.viewAll}</Link></header>
    <FailureSection state={state} retry={retry} copy={c} />
    {state.data && <>{reviewBuckets.every(bucket => state.data!.counts[bucket] === 0) ? <p>{c.emptyReviews}</p> : <div className="overview-review-groups">{reviewBuckets.map(bucket => state.data![bucket].length ? <section key={bucket} aria-labelledby={`overview-review-${bucket}`}><h3 id={`overview-review-${bucket}`}>{c[bucket]} ({state.data!.counts[bucket]})</h3><ul className="overview-action-list">{state.data![bucket].slice(0, 5).map(item => <li key={item.id} data-testid="overview-review-item"><div><strong>{item.targetType === 'diary' ? c.diary : c.thesis}</strong><span>{item.title}</span>{item.reviewDueAt && <time dateTime={item.reviewDueAt}>{c.due}: {formatInstant(item.reviewDueAt, locale, timezone, tx(c, 'unknown'))}</time>}</div><Link to={reviewHref(item)}>{c.open}</Link></li>)}</ul></section> : null)}</div>}</>}
  </section>
}

...
function ReviewSection({ state, retry, timezone, locale, c }: { state: ResourceState<ReviewGroups>; retry: () => void; timezone: string | null; locale: string; c: OverviewCopy }) {
  return <section className="overview-section" aria-labelledby="overview-review-title"><header className="overview-section-header"><h2 id="overview-review-title">{c.reviewQueue}</h2><Link to="/reviews">{c.viewAll}</Link></header>
    <FailureSection state={state} retry={retry} copy={c} />
...
function RecentSection({ state, retry, c }: { state: ResourceState<z.infer<typeof diaryListResponseSchema>>; retry: () => void; c: OverviewCopy }) {
  return <section className="overview-section" aria-labelledby="overview-recent-title"><header className="overview-section-header"><h2 id="overview-recent-title">{c.recent}</h2><Link to="/timeline">{c.viewAll}</Link></header><p className="overview-section-intro">{c.recentHint}</p>
    <FailureSection state={state} retry={retry} copy={c} />
...
function PlansSection({ state, retry, locale, timezone, c }: { state: ResourceState<z.infer<typeof tradePlanListResponseSchema>>; retry: () => void; locale: string; timezone: string | null; c: OverviewCopy }) {
  ...
  return <section className="overview-section" aria-labelledby="overview-plans-title"><header className="overview-section-header"><h2 id="overview-plans-title">{c.plans}</h2><Link to="/trade-plans">{c.viewAll}</Link></header><p className="overview-section-intro">{c.plansHint}</p>
    <FailureSection state={state} retry={retry} copy={c} />
...
function WatchlistSection({ state, retry, c }: { state: ResourceState<z.infer<typeof stockWatchlistResponseSchema>>; retry: () => void; c: OverviewCopy }) {
  return <section className="overview-section overview-side-section" aria-labelledby="overview-watchlist-title"><header className="overview-section-header"><h2 id="overview-watchlist-title">{c.watchlist}</h2><Link to="/stocks/watchlist">{c.viewAll}</Link></header><p className="overview-section-intro">{c.watchlistHint}</p><FailureSection state={state} retry={retry} copy={c} />

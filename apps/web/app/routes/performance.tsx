...
export default function Performance() {
  ...
  return <section className="performance-page"><h1>{c.title}</h1><p className="lede">{c.hint}</p><form className="performance-filters" onSubmit={submit}><label>{c.period}<select value={period} onChange={event => setPeriod(event.target.value)}>{(['month','quarter','year'] as const).map(value => <option key={value} value={value}>{c[value]}</option>)}</select></label><label>{c.symbol}<input value={symbol} maxLength={32} placeholder={c.all} onChange={event => setSymbol(event.target.value)} autoCapitalize="characters"/></label><button>{c.apply}</button></form>
    {error ? <><FailureNotice failure={error}/>{error.code?.startsWith('AUTH_') && <Link to={signInPath('/strategy-performance')}>{t('login')}</Link>}<button onClick={() => retry(value => value + 1)}>{t('retry')}</button></> : !data ? <p role="status">{t('loading')}</p> : !data.summary.totalClosedTrades ? <><p>{c.empty}</p><Link to="/diaries/new">{c.record}</Link></> : <>
      <dl className="performance-summary">{(['totalRealizedPnL','totalClosedTrades','winRate','wins','losses','maxDrawdownPct','sharpe'] as const).map(key => <div key={key}><dt>{c[key]}</dt><dd className={key==='totalRealizedPnL'?marketClass(data.summary.totalRealizedPnL):key==='maxDrawdownPct'?drawdownClass(data.summary.maxDrawdownPct):undefined} data-testid={`performance-${key}`}>{key==='totalRealizedPnL'?signed(data.summary.totalRealizedPnL):key==='maxDrawdownPct'||key==='winRate'?percent(data.summary[key]):number(data.summary[key],['totalClosedTrades','wins','losses'].includes(key)?0:2)}</dd></div>)}<div><dt>{c.best}</dt><dd>{data.bestStrategy?.name ?? c.none}</dd></div><div><dt>{c.worst}</dt><dd>{data.worstStrategy?.name ?? c.none}</dd></div></dl><p>{c.limits}</p><p>{c.utc}</p>
      ...
      <section><h2>{c.curve}</h2><PerformanceChart title={c.curve} points={data.equityCurve.map(row=>({label:row.date,value:row.cumPnL}))}/><details><summary>{c.data}</summary><PerformanceTable key={query} title={c.curve} headers={[c.date,c.pnl]} rows={data.equityCurve.map(row=>[row.date,<span className={marketClass(row.cumPnL)}>{signed(row.cumPnL)}</span>])}/></details></section>

...
export default function EtfResearch(){
 ...
 return <section className="plan-page"><ToolShell tool={toolByHref('/tools/etf')} title={c.title} intro={c.intro} actions={<Link className="secondary button" to="/etf/watchlist">{c.watch}{session.authenticated === true ? '' : ` · ${locale === 'en' ? 'Sign in to save' : locale === 'zh-CN' ? '登录后保存' : '登入後保存'}`}</Link>} />

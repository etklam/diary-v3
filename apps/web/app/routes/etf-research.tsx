...
export default function EtfResearch(){
 ...
 const number=(value:number|null,suffix='')=>value===null?c.unknown:`${new Intl.NumberFormat(locale,{maximumFractionDigits:2}).format(value)}${suffix}`;
 ...
 return <section className="plan-page"><header className="plan-header"><div><h1>{c.title}</h1><p className="lede">{c.intro}</p></div><Link to="/etf/watchlist">{c.watch}</Link></header>
 ...
 {[[c.risk,0,9],[c.valuation,9,14],[c.rs,14,17]].map(([title,start,end])=><section className="etf-section" key={title}><h3>{title}</h3><dl className="etf-metrics">{metrics.slice(Number(start),Number(end)).map((value,i)=>{const index=Number(start)+i;return <div key={index}><dt>{c.labels[index]}</dt><dd>{number(value??null,[2,3,4,5,6,7,10,13,14,15].includes(index)?'%':'')}{index===9&&data.valuation.currency?` ${data.valuation.currency}`:''}</dd></div>;})}</dl>{start===14&&<p>{data.benchmark} · {data.rs.from??c.unknown} – {data.rs.to??c.unknown}</p>}</section>)}

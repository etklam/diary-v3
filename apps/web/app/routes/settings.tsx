...
const copy = {
  'zh-TW': { title:'偏好設定', intro:'設定日記使用的語言、日期時區、預設工作頁與投資目標。', profile:'個人偏好', name:'名稱', language:'帳戶語言', startPage:'預設工作頁', startPageHint:'下次登入且沒有指定返回頁面時，會開啟這個頁面。', diaryLibrary:'日記庫', timeline:'時間軸', calendar:'日曆', timezone:'日期時區', deviceTimezone:'使用裝置時區', timezoneHint:'日記預設日期使用此時區，不會跟隨瀏覽器改變。例如 Asia/Taipei 或 America/New_York。', trades:'每月預期交易次數', profit:'預期獲利金額', holding:'預期平均持倉金額', targets:'投資目標', holidays:'統計時排除美股休市日', holidaysHint:'啟用後，Calendar 覆蓋率會排除週末與美股全日休市日；提早收市日仍可計入。帳戶時區只控制今天與日期顯示，不決定休市日；日記日期仍可使用。', save:'儲存設定', saved:'設定已儲存。', login:'請先登入，再管理個人偏好。', exact:'金額會儲存至小數點後兩位。', security:'管理密碼與裝置' },
  'zh-CN': { title:'偏好设置', intro:'设置日记使用的语言、日期时区、默认工作页与投资目标。', profile:'个人偏好', name:'名称', language:'账户语言', startPage:'默认工作页', startPageHint:'下次登录且没有指定返回页面时，会打开此页面。', diaryLibrary:'日记库', timeline:'时间轴', calendar:'日历', timezone:'日期时区', deviceTimezone:'使用设备时区', timezoneHint:'日记默认日期使用此时区，不会跟随浏览器改变。例如 Asia/Taipei 或 America/New_York。', trades:'每月预期交易次数', profit:'预期盈利金额', holding:'预期平均持仓金额', targets:'投资目标', holidays:'统计时排除美股休市日', holidaysHint:'启用后，Calendar 覆盖率会排除周末与美股全日休市日；提前收市日仍可计入。账户时区只控制今天与日期显示，不决定休市日；日记日期仍可使用。', save:'保存设置', saved:'设置已保存。', login:'请先登录，再管理个人偏好。', exact:'金额会保存至小数点后两位。', security:'管理密码与设备' },
  en: { title:'Preferences', intro:'Choose your diary language, date timezone, workspace start page, and investment targets.', profile:'Personal preferences', name:'Name', language:'Account language', startPage:'Start page', startPageHint:'Used after sign-in when no return page is specified.', diaryLibrary:'Diary library', timeline:'Timeline', calendar:'Calendar', timezone:'Date timezone', deviceTimezone:'Use device timezone', timezoneHint:'New diary dates use this timezone, independently of your browser. For example, Asia/Taipei or America/New_York.', trades:'Expected monthly trades', profit:'Expected profit amount', holding:'Expected average holding amount', targets:'Investment targets', holidays:'Exclude US market closed days from statistics', holidaysHint:'When enabled, Calendar coverage excludes weekends and full US market closures; early-close half-days remain eligible. Your account timezone controls today and date display, not closure selection; diary dates remain accessible.', save:'Save preferences', saved:'Preferences saved.', login:'Sign in to manage your preferences.', exact:'Amounts are saved to two decimal places.', security:'Manage password and devices' },
};
type FormValues = { name:string; locale:'zh-TW'|'zh-CN'|'en'; defaultWorkspacePage:'diaries'|'timeline'|'calendar'; timezone:string; expectedMonthlyTrades:string; expectedProfit:string; expectedAvgHolding:string; excludeHolidaysInStats:boolean };
      ...
      :auth==='loading'?<p role="status">{t('loading')}</p>:auth==='error'?<><p role="alert">{t('connection')}</p><button onClick={()=>void load()}>{t('retry')}</button></>
        :form&&<form onSubmit={save} aria-busy={pending}>
          <fieldset disabled={pending}><legend>{text.profile}</legend>
            ...
            <label>{text.startPage}<select name="defaultWorkspacePage" aria-describedby="start-page-hint" value={form.defaultWorkspacePage} onChange={e=>update('defaultWorkspacePage',e.target.value as FormValues['defaultWorkspacePage'])}><option value="timeline">{text.timeline}</option><option value="diaries">{text.diaryLibrary}</option><option value="calendar">{text.calendar}</option></select></label>
            <p className="muted" id="start-page-hint">{text.startPageHint}</p>

...
function formatNumber(locale: Locale, value: number) {
  return `${formatMarketValue(locale, value, 2)}%`
}
...
export default function Seasonality() {
  ...
  function monthPanel(label: string, month: SeasonalityMonth) {
    return <section className="market-research-section" aria-label={label}><h2>{label}: {seasonalityMonthName(month.month, locale)}</h2><dl className="market-ratio-read"><div><dt>{c.average}</dt><dd className={marketClass(month.avgReturn)}>{formatNumber(locale, month.avgReturn)}</dd></div><div><dt>{c.volatility}</dt><dd>{volatilityLabels[locale][month.volatility]}</dd></div><div><dt>{c.strength}</dt><dd>{strengthLabels[locale][seasonalityStrength(month.avgReturn)]}</dd></div></dl><p>{month.characteristics[locale]}</p><p className="market-research-meta">{c.reasons}: {month.reasons[locale].join(' · ')}</p></section>
  }

  return <section className="market-research-page">
    <ToolShell tool={toolByHref('/tools/seasonality')} title={c.title} intro={c.intro} actions={<button type="button" className="secondary" onClick={() => void copyMarkdown()}>{copyState === 'copied' ? c.copied : copyState === 'failed' ? c.copyFailed : c.copy}</button>} />{copyFallback && <textarea readOnly className="market-research-copy-fallback" aria-label={c.copyFailed} value={copyFallback} />}
    ...
    <section className="market-research-section" aria-labelledby="seasonality-chart-title"><h2 id="seasonality-chart-title">{c.monthly}</h2><div className="seasonality-chart" tabIndex={0} aria-label={c.barChart}><svg viewBox="0 0 840 280" role="img" aria-label={c.barChart}><line x1="420" y1="18" x2="420" y2="252" />{Array.from({ length: 12 }, (_, index) => { const month = seasonalityMonthData(index + 1)!; const y = 20 + index * 19; const width = Math.abs(month.avgReturn) / maxMagnitude * 360; return <g key={month.month}><text x="8" y={y + 12}>{seasonalityShortMonthName(month.month, locale)}</text><rect className={marketClass(month.avgReturn)} x={month.avgReturn >= 0 ? 420 : 420 - width} y={y} width={width} height="13" /><text x={month.avgReturn >= 0 ? 426 + width : 414 - width} y={y + 12}>{formatNumber(locale, month.avgReturn)}</text></g> })}</svg><p className="market-research-meta">{c.zero}</p></div><div className="seasonality-table"><table><caption>{c.monthly}</caption><thead><tr><th scope="col">{c.month}</th><th scope="col">{c.average}</th><th scope="col">{c.volatility}</th><th scope="col">{c.strength}</th><th scope="col">{c.characteristics}</th><th scope="col">{c.reasons}</th></tr></thead><tbody>{Array.from({ length: 12 }, (_, index) => seasonalityMonthData(index + 1)!).map(month => <tr key={month.month} className={month.month === currentMonth ? 'current' : undefined}><th scope="row">{seasonalityMonthName(month.month, locale)}</th><td className={marketClass(month.avgReturn)}>{formatNumber(locale, month.avgReturn)}</td><td>{volatilityLabels[locale][month.volatility]}</td><td>{strengthLabels[locale][seasonalityStrength(month.avgReturn)]}</td><td>{month.characteristics[locale]}</td><td>{month.reasons[locale].join(' · ')}</td></tr>)}</tbody></table></div></section>
    <section className="market-research-section"><h2>{c.interpretation}</h2><div className="market-research-grid"><div><h3>{c.best}</h3><p>{monthList(strongest, locale)}</p><p>{period.strongDescription}</p><p>{c.periodAverage}: <span className={marketClass(strongAverage)}>{formatNumber(locale, strongAverage)}</span></p></div><div><h3>{c.worst}</h3><p>{monthList(weakest, locale)}</p><p>{period.weakDescription}</p><p>{c.periodAverage}: <span className={marketClass(weakAverage)}>{formatNumber(locale, weakAverage)}</span></p></div></div><div className="seasonality-interpretation"><p>{period.strongStrategy}</p><p>{period.weakStrategy}</p></div></section>

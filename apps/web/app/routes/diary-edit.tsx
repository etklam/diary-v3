...
export default function EditDiary(){const {id}=useParams();const {t,locale}=useUi();const labels=diaryCopy[locale];const [entry,setEntry]=useState<DiaryFields|null>(null);const [error,setError]=useState<Failure|null>(null);const [accountId,setAccountId]=useState('');
 ...
 return <section className="editor"><h1>{labels.edit}</h1>{error?<><FailureNotice failure={error}/><button onClick={()=>void load()}>{t('retry')}</button><Link className="inline-link" to={signInPath(`/diaries/${id}/edit`)}>{t('login')}</Link></>:entry?<DiaryEditor key={id} initial={entry} id={id} accountId={accountId||undefined}/>:<p role="status">{t('loading')}</p>}</section>;
}

...
export default function DiaryReviewPage(){const {id=''}=useParams(),{locale,t}=useUi(),c=reviewCopy[locale],original=diaryCopy[locale],session=useSessionState();const [review,setReview]=useState<Review|null>(null),[timezone,setTimezone]=useState(''),[loading,setLoading]=useState(true),[loadError,setLoadError]=useState<Failure|null>(null),[attempt,setAttempt]=useState(0),[editing,setEditing]=useState(false),[saving,setSaving]=useState(false),[form,setForm]=useState<Form>({reviewOutcome:'',reviewSummary:'',reviewLearning:'',reviewAdjustment:''}),[error,setError]=useState<Failure|null>(null),[saved,setSaved]=useState(false);const [saveState,setSaveState]=useState<'idle'|'saving'|'failed'>('idle'),[accountId,setAccountId]=useState(''),[baseline,setBaseline]=useState<ReviewBaseline|null>(null),[restorable,setRestorable]=useState<ReviewDraft|null>(null),[draftClosed,setDraftClosed]=useState(false);const active=useRef(true),completedHeading=useRef<HTMLHeadingElement>(null);const draftKey=accountId?`review-draft:${accountId}:${id}`:null;
 ...
 useEffect(()=>{if(!draftKey||restorable||draftClosed||!dirty)return;const timer=setTimeout(()=>{try{localStorage.setItem(draftKey,JSON.stringify({at:Date.now(),value:stateRef.current.form}));}catch{/* Recovery is best effort; storage may be unavailable. */}},600);return()=>clearTimeout(timer);},[draftKey,restorable,draftClosed,dirty,form]);
 ...
 // reflection stays recoverable after re-login.
 useEffect(()=>()=>{const {form:current,draftKey:key,restorable:pending,draftClosed:closed}=stateRef.current;if(!key||closed||pending||!dirtyRef.current)return;try{localStorage.setItem(key,JSON.stringify({at:Date.now(),value:current}));}catch{/* Recovery is best effort. */}},[draftKey]);
 // An explicit signed-out session invalidates the device draft; a 401 expiry never does.
 useEffect(()=>{if(session.authenticated===false){clearDraft();setRestorable(null);}},[session.authenticated]);
 useEffect(()=>{active.current=true;const controller=new AbortController();setLoading(true);setReview(null);setLoadError(null);Promise.all([api.GET('/api/diaries/{id}/review',{params:{path:{id}},signal:controller.signal}),api.GET('/api/auth/me',{signal:controller.signal})]).then(([result,user])=>{if(!active.current||controller.signal.aborted)return;if(!result.response.ok){setLoadError(apiFailure(result.response.status===401?{data:{code:'AUTH_UNAUTHORIZED'}}:result.error,t('failed')));return;}if(!user.response.ok||!user.data){setLoadError(apiFailure(user.error,t('connection')));return;}const data=diaryReviewResponseSchema.parse(result.data);const confirmed=baselineFrom(data);setReview(data);setForm(formFrom(data));setBaseline(confirmed);setTimezone(user.data.data.timezone);setEditing(false);setSaveState('idle');setError(null);setSaved(false);setAccountId(user.data.data.id);setRestorable(restorableDraft(readReviewDraft(`review-draft:${user.data.data.id}:${id}`),confirmed,data));setDraftClosed(false);}).catch(()=>{if(active.current&&!controller.signal.aborted)setLoadError(apiFailure(null,t('connection')));}).finally(()=>{if(active.current&&!controller.signal.aborted)setLoading(false);});return()=>{active.current=false;controller.abort();};},[id,attempt]);

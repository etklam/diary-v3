...
export default function Discipline() {
 ...
 return <section className="plan-page discipline-page"><h1>{c.title}</h1><p className="lede">{c.intro}</p>{error ? <><FailureNotice failure={error}/>{error.code?.startsWith('AUTH_') && <Link to={signInPath(location.pathname + location.search)}>{t('login')}</Link>}<button onClick={() => retry(value => value + 1)}>{t('retry')}</button></> : rows === null ? <p role="status">{t('loading')}</p> : <>
  ...
  <FailureNotice failure={writeError}/>{saved && <p role="status">{c.saved}</p>}<button className="secondary discipline-draw-button" disabled={pending} onClick={draw}>{c.draw}</button>{drawn && <div role="status" className="discipline-draw"><p className="discipline-draw-label">{drawn.isCustom ? c.custom : c.fallback}</p><p className="discipline-sentence" style={{ overflowWrap: 'anywhere', whiteSpace: 'pre-wrap' }}>{drawn.content}</p></div>}
  <h2>{c.list}</h2>{!rows.length ? <p>{c.empty}</p> : <ol className="discipline-list">{rows.map((row, index) => <li key={row.id} data-testid="principle"><p id={`principle-${row.id}`} tabIndex={-1} className="discipline-sentence" style={{ overflowWrap: 'anywhere', whiteSpace: 'pre-wrap' }}>{row.content}</p><p className="muted discipline-meta">{c.created}: <time dateTime={row.createdAt}>{new Intl.DateTimeFormat(locale, { dateStyle: 'medium', timeStyle: 'short', timeZone: timezone }).format(new Date(row.createdAt))}</time></p><div className="actions"><button className="secondary" disabled={pending || editing !== null || dirty.current || index === 0} onClick={() => move(index, -1)}>{c.up}</button><button className="secondary" disabled={pending || editing !== null || dirty.current || index === rows.length - 1} onClick={() => move(index, 1)}>{c.down}</button><button className="secondary" disabled={pending || editing !== null || dirty.current} onClick={() => { setEditing(row.id); setContent(row.content); setWriteError(null); }}>{c.edit}</button><button className="secondary" disabled={pending || editing !== null || dirty.current} onClick={() => remove(row.id)}>{c.remove}</button></div></li>)}</ol>}

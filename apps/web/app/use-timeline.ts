...
export function useTimeline(queryString:string){const {t}=useUi(),message=useRef(t);message.current=t;const [state,setState]=useState<State>(initial),generation=useRef(0),flight=useRef<AbortController|null>(null),busy=useRef(false);
 ...
 try{const result=await api.GET('/api/diaries/summary',{params:{query:parsed.data},signal:controller.signal});if(epoch!==generation.current)return;if(result.response.ok&&result.data){const response=diarySummaryListResponseSchema.parse(result.data);setState(current=>({...current,entries:mergeTimelineEntries(append?current.entries:[],response.data.map(projectSummary)),page,totalPages:response.pagination.totalPages,error:null}));}else setState(current=>({...current,error:apiFailure(result.response.status===401?{data:{code:'AUTH_UNAUTHORIZED'}}:result.error,message.current('failed'))}));}catch(error){if(epoch===generation.current&&!controller.signal.aborted)setState(current=>({...current,error:apiFailure(error,message.current('connection'))}));}finally{if(epoch===generation.current){busy.current=false;setState(current=>({...current,loading:false,loadingMore:false}));}}},[queryString]);

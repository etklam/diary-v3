...
type RecoveryDraft = Omit<Draft, 'status'>
...
function recoveryDraft(draft: Draft): RecoveryDraft {
  const { title, content, excerpt, coverImage, category, tags } = draft
  return { title, content, excerpt, coverImage, category, tags }
}
...
function storedDraft(value: unknown): RecoveryDraft | null {
  ...
  return recoveryDraft(item as Draft)
}
...
export function AdminPostEditor({ id }: { id?: string }) {
  ...
  function update(field: keyof Draft, value: string) { setSuccess(null); if (field === 'coverImage') setCoverFailed(false); setDraft(current => ({ ...current, [field]: value })) }

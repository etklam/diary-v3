 ...
 return <section className="ledger-reading portfolio-valuation" aria-labelledby="portfolio-title"><h2 id="portfolio-title">{c.title}</h2>
  {error?<><FailureNotice failure={error} id="portfolio-error"/><button className="secondary" onClick={()=>retry(value=>value+1)}>{t('retry')}</button>{error.code?.startsWith('AUTH_')&&<Link to={signInPath('/stocks')}>{t('login')}</Link>}</>:!data?<p role="status">{t('loading')}</p>:<>
   ...
   {data.valuation.valuationStatus!=='empty'&&<><p className="muted">{c.hint}</p><dl>
    {([['value','currentMarketValue'],['pnl','unrealizedAmount'],['pct','unrealizedPct'],['cost','pricedCostBasis'],['unpriced','unpricedCostBasis'],['coverage','quoteCoveragePct']] as const).map(([label,key])=><div key={key}><dt>{c[label]}</dt><dd className={key==='unrealizedAmount'||key==='unrealizedPct'?marketClass(data.valuation[key]):undefined} data-testid={`valuation-${key}`}>{key==='unrealizedAmount'||key==='unrealizedPct'?signed(data.valuation[key]):number(data.valuation[key])}{(key==='unrealizedPct'||key==='quoteCoveragePct')&&data.valuation[key]!==null?'%':''}</dd></div>)}
   ...
   <div className="holdings-table"><table aria-label={c.title}><thead><tr><th>{c.symbol}</th><th>{c.price}</th><th>{c.position}</th><th>{c.pnl}</th></tr></thead><tbody>{data.holdings.map(holding=>{const unrealized=holding.price===undefined?null:holding.price*holding.quantity-holding.totalCost;return <tr key={holding.symbol}><th scope="row"><Link to={`/stocks/${encodeURIComponent(holding.symbol)}`}>{holding.symbol}</Link></th><td>{number(holding.price??null)}</td><td>{number(holding.price===undefined?null:holding.price*holding.quantity)}</td><td className={marketClass(unrealized)}>{signed(unrealized)}</td></tr>})}</tbody></table></div></>}
  </>}
 </section>;
}

...
| SMA/EMA seeds, RMA reseeding and slopes | 03 | `calculateSmaSeries`, `calculateEmaSeries`, `calculateRmaSeries`, `calculateResearchMetrics` | Golden seed and missing-value tests | Review first values, warm-up and source scale |
| RSI14, MACD12/26/9, TR/ATR14 | 03 | `calculateResearchMetrics` | Golden warm-up index tests | Review terminal close, one TR and indicator basis |
| DMI/ADX14 and zero denominators | 03 | `calculateResearchMetrics` | ADX first-value test; zero-safe arithmetic | Review direction versus strength interpretation |
| Bollinger20/2 using population standard deviation | 03 | `calculateResearchMetrics` | Warm-up and deterministic output tests | Confirm close series and rounding at report boundary |

...
| Focused browser acceptance | `npx playwright test tests/e2e/research-diary-handoff.spec.ts` | Passed, 18 tests; full suite including Company CTA, context notice, Full flow, parser notice, and visual captures |
| Built artifact acceptance | `npx playwright test --config=playwright.release.config.ts` | Passed, 4 tests; includes the Company capture/append/return flow against built artifacts |

## Limitations

The pending Company CTA, contextual notice and saved-detail return link landed in commit `c21442c` ("feat: land company capture CTAs and research return handoff") and were verified with the full handoff suite, affected regression suites and built-artifact acceptance; the evidence screenshots were opened and inspected at 1440px, 390px and dark themes. A generated screenshot file is not treated as visual acceptance until it has been opened and inspected for layout, contrast, wrapping, and data-boundary defects.

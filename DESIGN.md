...
The workspace navigation starts with Overview, then groups primary destinations under Diary & review (Diary library, Timeline, Calendar, Review queue), Investing & trading (Holdings, Watchlist, Trade plans), and Markets & tools (Market research, Tools). Diary library, Timeline, and Calendar are direct, equal-priority links in the desktop sidebar and each has a clear icon and text label. Plain sign-in opens the user's selected start page, defaulting to Timeline; an explicit safe return destination takes priority. Capture remains a separate primary action with an adjacent full-diary disclosure. Diary reminders and Partners live in Diary management; Price reminders and Trading principles live in Trade management. Account contains Public articles and Settings. Administration orders Article management, User management, then ETF catalog. Ordinary links and native disclosures retain the browser keyboard model, without application-menu roles.

Below 768px, the brand/Menu header stays at the top and a fixed four-column bottom bar holds Diary library, Timeline, Calendar, and Write diary. Write diary opens `/diaries/new`; both new and quick capture routes select the writing destination. Library remains current for diary listing, reading, and editing. The drawer retains Quick Diary and the remaining workspace links; full-diary writing moves into the bottom bar. The bar uses existing icon/label styling, safe-area padding, and reserved main-content clearance; full-page quick-save controls sit above it while modal dialogs retain their own footer. Each direct route stays a separate page. Timeline is current at `/timeline` and Partner comparison; Calendar is current at `/calendar`; Review queue is current for Diary Review. Each current route uses `aria-current=page` and the selected surface treatment. The skip link appears on focus; a pathname change moves focus to main, and main has a 4px inset keyboard outline. The preview's three toggle buttons use `aria-pressed`; they are not full business navigation.

Diary library, Timeline, and Calendar use the available workspace main-column width so the large desktop viewport does not leave a broad unused rail. Keep Diary excerpts and other reading text bounded to 72–75ch; reading pages and editors retain their existing widths. The three core browse links remain one click away from every workspace page.
...
The article editor makes its stored state explicit beside the title and exposes one filled primary action per state. New articles default to Draft with MEMBER access. Draft uses Save draft with a separate Publish action; Published uses Update article with a separate Archive article action; Archived uses Save changes with a separate Republish action. A labelled native PUBLIC/MEMBER select beside the publication controls defines who can read after publication, with a short explanatory hint; it becomes full-width above the actions on mobile. The optional public teaser is authored separately from the article body. Successful article links always come from the server response and appear only for Published. Failed or expired sessions keep bounded, account-and-article-scoped recovery data. Published articles use the public reading shell for guests, ordinary users, and Admins; Admins receive only contextual New, Manage, and Edit links. Lists identify Public or Members only access in text. A locked MEMBER reader shows the authored teaser, a Members only heading, and Sign in/Create account actions, using the existing neutral palette, blue action color, 16px/24px spacing, and reading typography of 1rem/1.85 capped at 72ch.

### Personal achievements

Personal achievements uses an 800px bounded page within the shared workspace gutter and existing semantic light/dark theme. The newest-first chronology pairs quiet tabular dates with larger achievement text and secondary Edit/Delete actions, separated by thin rules in one surface card. At widths of 767px and below, each entry becomes a single column with wrapping content and left-aligned actions. One filled primary action changes from Add achievement to Save achievement while the inline form is open; the form uses a labelled native date input and resizable text area. Opening create or edit focuses the date field, and cancelling or saving successfully returns focus to Add achievement. Failed saves preserve input and the shared `FailureNotice` receives focus; pending controls are disabled and completion uses a text status. The direction is recorded in `docs/design/personal-achievements-brief.md`.

### Quick Diary

The native dialog is `min(680px, calc(100% - 32px))` wide on desktop, at most `calc(100dvh - 48px)` tall, with 24px padding; on mobile it is full viewport height with 16px padding. The footer sticks to the bottom edge of the dialog and gains safe-area on mobile. On the full `/diaries/quick` page the submit button is sticky above the bottom navigation on phones — it keeps its own layout slot with a surface background and an upward shadow, so it never covers content that cannot be scrolled past. Opening moves focus to the text area; Escape closes and returns focus to the trigger. It reuses the real create-diary form; date and save-mode stay side by side on phones.
...
Mobile diary navigation has its own bottom bar, leaving the brand and Menu in the compact top header. See `docs/design/security-fire-finish-review.md` for the actual acceptance and screenshots; follow-up acceptance for full settings and diary Markdown editing is recorded separately in tickets 07 and 08.

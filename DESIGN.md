...
On mobile, the brand/Menu header is followed by a persistent three-column shortcut row for Diary library, Timeline, and Calendar. The same destinations are omitted from the drawer to avoid duplicates. Each direct route stays a separate page. Diary library is current throughout `/diaries` and its reading/editing routes; Timeline is current at `/timeline` and Partner comparison; Calendar is current at `/calendar`; Review queue is current for Diary Review. Each current route uses `aria-current=page` and the selected surface treatment. The skip link appears on focus; a pathname change moves focus to main, and main has a 4px inset keyboard outline. The preview's three toggle buttons use `aria-pressed`; they are not full business navigation.
